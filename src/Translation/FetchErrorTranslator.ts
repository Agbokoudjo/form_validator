...
import { Logger, detectLanguageFromDom, hasProperty } from "../_Utils";
import {
 ...
 */
const DEFAULT_CONFIG: Required<Omit<FetchErrorTranslatorConfig, 'customPatterns' | 'additionalTranslations'>> = {
    defaultLanguage: 'en',
 ...
 * const frenchMessage = fetchErrorTranslator.translate(error.name, error, 'fr');
 ...
 */
export class FetchErrorTranslator implements ConfigCacheAdapterTranslation {
    /**
     * Singleton instance
     * @private
     */
    ...
    constructor(config: FetchErrorTranslatorConfig = {}) {
        ...
        // Detect language
        this.currentLanguage = detectLanguageFromDom(this.config.defaultLanguage!);
     ...
     * ```typescript
     * // Simple translation
     * const message = translator.translate('NetworkError');
     * 
     * // With error object for pattern matching
     * try {
     *     await fetch('/api/data');
     * } catch (error) {
     *     const message = translator.translate(error.name, error);
     *     console.error(message);
     * }
     * 
     * // With specific language
     * const frenchMessage = translator.translate('AbortError', null, 'fr');
     * ```
     */
    public translate(errorName: string, error?: Error | null, language?: string): string {
    ...
    private getSSLErrorMessage(language: string): string {
        ...
        return sslMessages[language] || sslMessages.en;
    }
    ...
    private logDebug(message: string, data?: unknown): void {
        if (this.config.debug) {
            Logger.log(`[FetchErrorTranslator] ${message}`, data || '');
        }
    }
    ...
    private logError(message: string, error: unknown, context?: Record<string, unknown>): void {
        Logger.error(`[FetchErrorTranslator] ${message}`, {
