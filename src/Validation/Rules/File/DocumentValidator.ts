 ...
 */
export class DocumentValidator extends AbstractMediaValidator {
    ...
    public validate = async (medias: File | FileList, targetInputname: string, optionsdoc: OptionsFile): Promise<this> => {
        ...
        for (const filepdf of files) {
            ...
            if (['xlsx,xls'].includes(extension_file ?? 'xls')) {
            ...
            const signatureErrorPdf = await this.filevalidate(filepdf, format_validator);
    ...
    private async filevalidate(file: File, formatValidator: string): Promise<string | null> {
        ...
        switch (formatValidator) {
            ...
            default:
                console.warn(`Unsupported file format: ${formatValidator}`);
                return null;
        }
    }
