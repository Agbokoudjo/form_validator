 ...
 */
export class DateInputValidator extends AbstractFieldValidator implements DateInputValidatorInterface {
    ...
    public validate = (date_input: string | Date, targetInputname: string, date_options: DateInputOptions = {}): this => {
        const {
            format = 'YYYY/MM/DD',
            strictMode,
            delimiters = ['/', '-'],
        ...
        if (typeof date_input === "string") {
            ...
            if (strictMode === true && date_input.length !== format.length) {
                return this.setValidationState(false, 'Date does not match required length in strict mode.', targetInputname);
            }
            ...
            if (!formatDelimiter || !dateDelimiter) {
                return this.setValidationState(false, 'Could not detect delimiter in date.', targetInputname);
            }

            const dateParts = date_input.split(dateDelimiter);
            const formatParts = format.toLowerCase().split(formatDelimiter);
            if (dateParts.length !== formatParts.length) {
                return this.setValidationState(false, 'Mismatch between date and format parts.', targetInputname);
            }
            ...
            for (const [value, format] of zipped_date) {
                ...
                if (!value || !format || value.length !== format.length) {
                    return this.setValidationState(false, `Invalid part: '${value}' does not match '${format}'`, targetInputname);
                }
            ...
            if (fullYear.startsWith('-')) {
                return this.setValidationState(false, 'Negative years are not supported.', targetInputname);
            }
            ...
            if (isNaN(dateValue.getTime())) {
                return this.setValidationState(false, 'Invalid date created from input.', targetInputname);
            }

            //La méthode getUTCDate() renvoie le jour du mois pour la date renseignee d'apres UTC.
            if (dateValue.getUTCDate() !== +day) {
                return this.setValidationState(false, 'Day mismatch (invalid day in month).', targetInputname);
            }

            // Vérification de la date minimale
            if (minDate && dateValue < minDate) {
                return this.setValidationState(false, `The date must be after ${minDate}.`, targetInputname);
            }
            // Vérification de la date maximale
            if (maxDate && dateValue > maxDate) {
                return this.setValidationState(false, `The date must be before ${maxDate}.`, targetInputname);
            }

            const now = new Date();
            // Vérification si les dates futures sont autorisées
            if (allowFuture === false && dateValue > now) {
                return this.setValidationState(false, `The date "${dateValue.toISOString().split('T')[0]}" cannot be in the future.`, targetInputname);
            }

            // Vérification si les dates passées sont autorisées
            if (allowPast === false && dateValue < now) {
                return this.setValidationState(false, `The date "${dateValue.toISOString().split('T')[0]}" cannot be in the past.`, targetInputname);
            }
        }
        return this;
    }
