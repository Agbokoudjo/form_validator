...
import { checkHost, deepMergeAll, isIP, CustomURL } from "../../_Utils";
import { FormError } from "../FormError";
import { textInputValidator } from "./TextInputValidator";
export interface URLOptions extends FQDNOptions {
...
export interface URLInputValidatorInterface {
     ...
     * });
     */
    urlValidator: (urlData: string, targetInputname: string, url_options: URLOptions) => Promise<this>;
}
 ...
 */
export class URLInputValidator extends FormError implements URLInputValidatorInterface {
    private static __instanceURLValidator: URLInputValidator | null = null;
    constructor() { super(); }
    ...
    public urlValidator = async (urlData: string, targetInputname: string, url_options: URLOptions): Promise<this> => {
        if (!urlData || /[\s<>]/.test(urlData) || urlData.indexOf('mailto:') === 0) {
            return this.setValidatorStatus(false, `The value "${urlData}" is not a valid URL format.`, targetInputname);
        }
        ...
        if (__UrlOptions.validateLength === true ||
            ...
            textInputValidator.textValidator(urlData, targetInputname, {
        ...
        let parsedURL: CustomURL;
        try {
            parsedURL = new CustomURL(urlData);
        } catch (error) {
            return this.setValidatorStatus(false, `The value "${urlData}" is not a valid URL.`, targetInputname);
        }
        const protocol = parsedURL.protocol.replace(':', '');
        const hostname = parsedURL.hostname;
        await fqdnInputValidator.fqdnValidator(hostname, targetInputname, {
        ...
        if (__UrlOptions.requireHost && (!hostname || hostname.trim() === '')) {
            return this.setValidatorStatus(false, `A hostname is required in the URL.`, targetInputname);
        }

        // Vérifie protocole obligatoire
        if (__UrlOptions.requireProtocol && !parsedURL.protocol) {
            return this.setValidatorStatus(false, `Protocol is required in the URL.`, targetInputname);
        }
        ...
        if (!__UrlOptions.allowLocalhost && (hostname === "localhost" || hostname === "127.0.0.1")) {
            return this.setValidatorStatus(false, `The hostname "${hostname}" is not allowed.`, targetInputname);
        }
        // IPs interdites
        if (!__UrlOptions.allowIP && (isIP(parsedURL.hostname, { version: 4 }) || isIP(parsedURL.hostname, { version: 6 }))) {
            return this.setValidatorStatus(false, `IP addresses (IPv4 or IPv6) are not allowed in URLs.`, targetInputname);
        }
        // Paramètres de requête interdits
        if (!__UrlOptions.allowQueryParams && parsedURL.search) {
            return this.setValidatorStatus(false, `Query parameters "${parsedURL.search}" are not allowed.`, targetInputname);
        }
        // Fragments (#) interdits
        if (!__UrlOptions.allowHash && parsedURL.hash) {
            return this.setValidatorStatus(false, `Fragments "${parsedURL.hash}" are not allowed.`, targetInputname);
        }
        // Disallow protocol-relative URLs if not allowed
        if (urlData.startsWith('//') && !__UrlOptions.allowProtocolRelativeUrls) {
            return this.setValidatorStatus(false, `Protocol-relative URLs (//example.com) are not allowed.`, targetInputname);
        }
        // Require port
        if (__UrlOptions.requirePort && !parsedURL.port) {
            return this.setValidatorStatus(false, `The URL must specify a port.`, targetInputname);
        }
        // Disallow auth (user:pass)
        if (__UrlOptions.disallowAuth && (parsedURL.username || parsedURL.password)) {
            return this.setValidatorStatus(false, `Authentication credentials in URLs are not allowed.`, targetInputname);
        }
        // Host blacklist
        if (__UrlOptions.hostBlacklist && checkHost(hostname, __UrlOptions.hostBlacklist)) {
            return this.setValidatorStatus(false, `The hostname "${hostname}" is blacklisted.`, targetInputname);
        }
        // Host whitelist
        if (__UrlOptions.hostWhitelist && !checkHost(hostname, __UrlOptions.hostWhitelist)) {
            return this.setValidatorStatus(false, `The hostname "${hostname}" is not in the allowed list.`, targetInputname);
        }
        return this;
    }
