import { HTMLFormChildrenElement, MediaType, AttributeException, Logger } from "../_Utils";
import {
    FormChildrenValidate, FormChildrenValidateInterface
} from "./FormChildrenValidate";
...
export class FormValidate {
    ...
    private init = (formCssSelector: string): void => {
    ...
    public validateChildrenForm = async (target: HTMLFormChildrenElement): Promise<void> => {
        ...
        try {
            const validator = new FormChildrenValidate(target);
            await validator.validate();
    ...
    public buildValidators(): FormChildrenValidateInterface[] {
        ...
        this.childrens.each((_i, el) => {
            const validator = new FormChildrenValidate(el as HTMLFormChildrenElement);
